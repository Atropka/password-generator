...
class App(CTk.CTk):
    def __init__(self):
        ...
        self.logo = CTk.CTkImage(dark_image=Image.open("img_3.png"), size=(150, 150))
        self.entry = CTk.CTkImage(dark_image=Image.open("img_1.png"))
        ...
        self.entry_password = CTk.CTkEntry(master=self.password_frame, width=300, placeholder_text='The configuration must be selected!')
        self.entry_password.grid(row=0, column=0, padx=(0, 20))

        self.btn_generate = CTk.CTkButton(master=self.password_frame, text="Generate", width=100, image=self.entry,
                                          command=self.set_password)
        self.btn_generate.grid(row=0, column=1)

        self.settings_frame = CTk.CTkFrame(master=self)
        self.settings_frame.grid(row=3, column=0, padx=(20, 20), pady=(20, 0), sticky="nsew")
        ...
        self.cb_digits.grid(row=3, column=0, padx=10)
        ...
        self.cb_lower.grid(row=3, column=1)
        ...
        self.cb_upper.grid(row=3, column=2)

        self.error_label = CTk.CTkLabel(master=self, text="")
        self.error_label.grid(row=2, column=0, padx=(20, 20), pady=(20, 0), sticky="nsew")
        ...
        self.cb_symbols.grid(row=3, column=3)

        self.appearance_mode_option_menu = CTk.CTkOptionMenu(self.settings_frame,
                                                             values=["Light", "Dark", "System"],
                                                             command=self.change_appearance_mode_event)
        self.appearance_mode_option_menu.grid(row=5, column=0, columnspan=4, pady=(10, 10))
    ...
    def set_password(self):
        ...
        else: self.error_label.configure(text='Please specify the configuration for generating a password!')
